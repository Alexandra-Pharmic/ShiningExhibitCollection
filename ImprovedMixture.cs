...
{
    public sealed class ImprovedMixtureDef : ExhibitTemplate
    {
        public override IdContainer GetId()
        {
            return nameof(ImprovedMixture);
        }
        ...
        public override ExhibitConfig MakeConfig()
        {
            var exhibitConfig = new ExhibitConfig(
                ...
                RelativeCards: new List<string>() { "Potion" }
            );
            return exhibitConfig;
        }
        [EntityLogic(typeof(ImprovedMixtureDef))]
        [UsedImplicitly]
        [ExhibitInfo(WeighterType = typeof(ImprovedMixtureWeighter))]
        public sealed class ImprovedMixture : ShiningExhibit
        {
            ...
            {
                NotifyActivating();
                yield return this.UpgradePotions(args.Cards);
                yield break;
            }
            ...
            private BattleAction UpgradePotions(IEnumerable<Card> cards)
            {
                foreach (Card card in cards)
                {
                    if (card is Potion && !(card.IsUpgraded))
                    {
                        card.Upgrade();
                    }
                }
            ...
            private class ImprovedMixtureWeighter : IExhibitWeighter
            {
